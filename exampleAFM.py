...
num_spins = 6
num_neighbors = 1 # specify number of neighbors for interaction
...
H_single = -2*J1*S(n).dot(S(n+1) + S(n-1)) - 2*S(n).dot(h) + K*Sz(n)**2
H_sum = -2*J1*S(n).dot(S(n+1)) - 2*S(n).dot(h) + K*Sz(n)**2
...
AFM_groundstate = np.array([[(-1)**i, 0, 0] for i in range(num_spins)])

groundstate=find_ground_state_python(H_num, coupling_constants=None, coupling_constants_n=coupling_constants_n, num_neighbors=num_neighbors, num_spins=num_spins, x0=AFM_groundstate)
#groundstate = AFM_groundstate
print(groundstate)
...
groundstates = []
groundstate_old = AFM_groundstate
